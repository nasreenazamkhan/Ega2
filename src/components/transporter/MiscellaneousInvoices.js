...
function MiscellaneousInvoices(props) {
  ...
  const validateInvoices = (invoiceList) => {
    ...
    for (let i = 0; i < invoiceList.length; i++) {
      ...
      if (inv.invoiceAmount==='' || !regexp.test(inv.invoiceAmount)) {
        inv.invoiceAmountError = true;
        validationSuccess = false;
      }
      if (inv.invoiceTotalAmount==='' || !regexp.test(inv.invoiceTotalAmount)) {
                  ...
                  <Grid item container xs={12} sm spacing={1}>
                    <Grid item xs={6} sm={2}>
                      ...
                      <TextField
                        ...
                        helperText={invoice.invoiceNumberError ? 'Please enter invoice number' : ''}
                        onBlur={(e) => {
                    ...
                    <Grid item xs={6} sm={2}>
                      ...
                      />
                      <span style={{color:'#FF0000',fontSize:'14px',fontFamily:'Dubai Regular'}}>{invoice.invoiceDateError ? 'Please enter valid Date' : ''}</span>
                    </Grid>
                    <Grid item xs={6} sm={2}>
                      <LabelHeader>
                        Invoice Amount
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            promiseData.then((result) => {
                              ...
                              else if (file.size<= 1500000 &&  invoice.invoiceDocSize <= 1500000) {
