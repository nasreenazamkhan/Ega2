  ...
  return (
    <>
      <Grid container style={{ marginTop: '16px' }}>
        <Grid item xs={12} style={{ marginBottom: 0 }}>
          <Breadcrumbs aria-label="breadcrumb" classes={{
            ...
            {location.state?.statusSelectedTab !== 'ALL' && <span style={{ color: '#848484' }}>
      ...
      {<div className="row" style={{ marginBottom: '0px' }}>
        <div className="col-md-12">
          {tabSelected === "Booking" &&
            <PaymentSummary statusData={location.state?.statusData} url={location.state?.url}>
            </PaymentSummary>}
          {tabSelected === "Miscellaneous" &&
            <MiscellaneousPayment  showRecords={location.state?.showRecords} statusData={location.state?.statusData} statusSelectedTab={location.state?.statusSelectedTab}>
