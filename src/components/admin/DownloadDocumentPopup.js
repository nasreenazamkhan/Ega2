      ...
      >
        ...
        <DialogContent style={{ paddingLeft: 0, paddingRight: 0, flex: 'none' }}>
          ...
          <Table>
            <TableBody>
              ...
              <TableRow>
                <StyledTableCell>
                  <Grid container style={{ paddingLeft: '20px', paddingTop: '20px', marginBottom: '0px !important' }}>
                    {props.fileList.map((file, index) => (<>
                      <Grid item xs={1} style={{ width: '40px' }}>
                        {(file.fileType === "JPG" || file.fileType === "jpg" || file.fileType === "jpeg") && <img src="./jpg_icon.svg" height="28px" />}
                        {file.fileType === "pdf" && <img src="./pdf_icon.svg" height="28px" />}
                        {(file.fileType === "doc" || file.fileType === "docx") && <img src="./doc_icon.svg" height="28px" />}
                        {(file.fileType === "png") && <img src="./png_icon.svg" height="28px" />}
                      </Grid>
                      <Grid item xs={4} style={{ fontSize: '14px', paddingBottom: '10px', paddingTop: '10px', minWidth: '160px' }}>{file.fileName}</Grid>
                      <Grid item xs={1} style={{ paddingRight: '20px' }}>
                        <IconButton style={{padding:'8px'}} onClick={() => downloadFile(file)}>
                          <img src="./pod_download.svg" />
                        </IconButton>
                      </Grid></>))}
                  </Grid>
                </StyledTableCell>
              ...
              {(['PODUPL', 'PODIMPAPPR'].includes(props.container.refStatus.code) && props.canApprove) && <><TableRow >
                <StyledTableCell style={{ paddingLeft: '40px', paddingRight: '40px' }}>
                  <CssTextField
                    placeholder="Review Comments (if any)"
                    multiline
                    rows={2}
                    variant="outlined"
                    fullWidth
        ...
        {(['PODUPL', 'PODIMPAPPR'].includes(props.container.refStatus.code) && props.canApprove) &&
          <DialogActions style={{ alignSelf: "center", marginBottom: '40px' }}>
            <StyledButton1
              style={{ color: '#FFFFFF', backgroundColor: '#1360D2', border: 'none', marginRight: '80px' }}
              variant="outlined"
              onClick={() => {
                props.onApprove(props.container)
              }}
            >
              Approve
            </StyledButton1>
            <StyledButton1
              variant="contained"
              onClick={() => {
                props.onReject(props.container)
              }}
            >
              Reject
            </StyledButton1>
