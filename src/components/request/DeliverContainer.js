...
const DeliverContainers = (props) => {
  ...
  useEffect(() => {
    ...
    };
    {
      props.addressData ? props.addressData.map((address, i) => {
        let opt = {
          label: `${address.selectedDropZoneLabel}, ${address.dropAddress}`,
          value: address.code,
        };
        addressOpt.push(opt);
      }) : addressOpt.push([])
  ...
  const RenderContactPerson = (props) => {
    ...
    if ((props.multiTimeFlag === true)) {
      ...
      props.containers.code = props.data?.code;
  ...
  const RenderLocation = (props) => {
    const updateDetails = (value) => {
      ...
      if (value == 'add') {
        ...
        // setAddressOpt([]);
        setLocation(value);
        props.containers.locationCode = value;
        {
          ...
          props.addressData.map((address, i) => {
            ...
            if (props.containers.locationCode === address.code) {
              ...
              props.containers.code = address.code;
              ...
              props.containers.dropAddress = address.dropAddress;

              // data.consigneeContactName = address.consigneeContactName;
              // data.consigneeContactNumber =
              //   address.consigneeContactNumber;
              // data.consigneeContactEmail = address.consigneeContactEmail;
              // data.phoneNumber = address.phoneNumber;
              // data.addressLine1 = address.addressLine1;
              // data.code = address.code;
              // data.addressCode = address.code;
              // data.addressNickname = address.addressNickname;
              // data.dropZone = address.selectedDropZone;
              // data.latLng = address.latLng;
              // data.dropAddress = address.dropAddress;
              setAddressPopup({ data: props.containers, showPopup: true, action: 'EDIT', cont: props.containers });
            }
          });
        }
      }
      ...
      return (
        <>
          <Select
            ...
            // defaultValue={props.containers.locationCode ? props.containers.locationCode : 'none'}
            ...
            onClick={(e) => {
              ...
              updateDetails(e.target.value ? e.target.value : e.target.dataset.value)
              props.handleUpdateLocation(props.containers)
            }}
          // onChange={(e) => {
          //   setMultiLocValue(()=>e.target.value)
        ...
        <>
          <BlueTooltip title={props.containers.dropAddress} arrow disableFocusListener={!props.containers.dropAddress} disableHoverListener={!props.containers.dropAddress}>
            <div>
              ...
              <span style={{ color: "#686868", paddingTop: "15px", maxWidth: "200px", minWidth: '120px', whiteSpace: 'nowrap', fontSize: '14px', fontFamily: 'Dubai Light', fontWeight: 600, paddingLeft: '5px' }}>
                {props.containers.dropAddress ? (props.containers.dropAddress.substring(0, 30) + "...") : "Select Location From Above"}
              </span>
            </div>
          </BlueTooltip>
        </>
      ...
      return (
        <>
          <OutlinedInput
            ...
            onChange={(e) => {
              props.containers.consigneeContactNumber = e.target.value
              props.containers.phoneNumber = e.target.value
            }}
          />
        </>
      )
        ...
        <form>
          <div style={{ borderRadius: '10px 10px 0px 0px', border: '1px solid #fff' }}>
            <Table aria-label="simple table" style={{ width: '100%' }}>
              ...
              <TableBody>
                {props.containers && props.containers.containerList && 
                  ...
                  <TableRow key={containers.container_number}>
                    <StyledTableCell>
                      <BlueTooltip arrow title={renderContainerTooltip(containers)}>
                        <div style={{
                          ...
                          <Avatar style={{ height: '25px', width: '25px', padding: '14px', marginRight: '8px', fontFamily: 'Dubai Light', backgroundColor: utils.randmonColorConsignee(), fontSize: '14px' }} > {containers.consigneeDetails.split("/")[1]}</Avatar>
                          <InputLabel
                            style={{ fontSize: '15px', color: "#0568AE", fontWeight: "bold", textDecoration: "underline", fontFamily: 'Dubai Light', paddingTop: '5px' }}
                          >
                    ...
                    <StyledTableCell>
                      <RenderLocation
                        ...
                        render={props.render}
                        handleUpdateLocation={props.handleUpdateLocation}
                      />
                    </StyledTableCell>
                    <StyledTableCell>
      ...
      {addressPopup.showPopup && (
        <CreateEditAddressPopup
          ...
          onConfirm={(address) => {
            ...
            //   value: e.code,
            // };
            // console.log(addressPopup.data)
            // if (addressPopup.action === 'ADD'){
            //   addressOpt.push(opt);
          // }
            // if (addressPopup.action === 'EDIT') {
            //   addressPopup.data.locationCode = e.code;
            //   addressPopup.data.dropAddress = e.dropAddress;
            //   addressPopup.data.consigneeContactName = e.consigneeContactName;
            //   addressPopup.data.consigneeContactNumber = e.consigneeContactNumber;
            //   addressPopup.data.consigneeContactEmail = e.consigneeContactEmail;
            //   addressPopup.data.phoneNumber = e.phoneNumber;
            //   addressPopup.data.addressLine1 = e.addressLine1;
            //   addressPopup.data.code = e.code;
            //   addressPopup.data.addressNickname = e.addressNickname;
            //   addressPopup.data.dropZone = e.dropZone;
            //   addressPopup.data.latLng = e.latLng;
            // }
            ...
            addressPopup.cont.consigneeContactName = address.consigneeContactName;
            addressPopup.cont.consigneeContactNumber = address.consigneeContactNumber;
            addressPopup.cont.locationCode = address.code;
            
            addressPopup.cont.consigneeContactEmail = address.consigneeContactEmail;
            addressPopup.cont.phoneNumber = address.phoneNumber;
            addressPopup.cont.addressLine1 = address.addressLine1;
            addressPopup.cont.code = address.code;
            addressPopup.cont.addressCode = address.code;
            addressPopup.cont.addressNickname = address.addressNickname;
            // addressPopup.cont.dropZone = address.selectedDropZone;
            addressPopup.cont.dropZone = address.dropZone;
            addressPopup.cont.latLng = address.latLng;
            addressPopup.cont.dropAddress = address.dropAddress;
            
            let action = addressPopup.action;
            setAddressPopup({ showPopup: false, cont:addressPopup.cont});
            props.onAddChange(action);
            setRender(render + 1);
          }}
        />
      )}
