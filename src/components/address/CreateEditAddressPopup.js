...
import { createStyles, FormControl, Grid, MenuItem, Select, withStyles } from "@material-ui/core";
...
const CreateEditAddressPopup = (props) => {
  ...
  const validationSchema = Yup.object({
    ...
    consigneeContactNumber: Yup.string()
      ...
      .length(12, (obj) => {return `Enter ${obj.length} digit mobile number`}),
    phoneNumber: Yup.string()
      .required("Phone Number is Required")
      .matches(/^[0-9]+$/,"Enter valid number")
      .matches(/^971/,"Number should start with 971")
      .length(11, (obj) => {return `Enter ${obj.length} digit phone number`}),
  ...
  const onSubmit = () => {
    ...
    if (address !== "" && methods.formState.isValid === true) {
      ...
      data.latLng = latLng;
      data.code = props.createEditFormData.code;
      ...
      postHttp(obj, true)
        .then((response) => {
          setPstate(pstate + 1);
          data.code = props.createEditFormData.code ? props.createEditFormData.code : response.data.dataItems[0].code
          props.action === "ADMIN_EDIT" ? props.onConfirm(address) : props.onConfirm(response.data.dataItems[0]);
