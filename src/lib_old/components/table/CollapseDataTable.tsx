...
import { Button, IconButton, Icon, TablePagination, TableSortLabel, TextField, Tooltip, CircularProgress,Checkbox } from '@material-ui/core';
import './table.css';
...
const getTableRow = ({ dk, row, index }: any) => {
    ...
    if (dk.type === COLUMN_TYPE_STRING)
        return (
            <StyledTableCell key={index} align="center">{datastr}</StyledTableCell>
        )
    else
        return (
            <StyledTableCell align="center" key={index}>{datastr}</StyledTableCell>
        )
...
const CollapseDataTable: React.FC<TableProps> = ({ tableData, tableKeys, handleClick, actions = [], remote, remoteUrl, refresh, dataRootKey,chkbox=false,collapsableTableKeys=[],collapseTableList,keyTest}) => {

    const [dtstate, setdtstate] = useState({ rowsPerPage: 10, page: 0, sortColumn: null, currentPageRows: [], currentPageKey: [], refresh: 0,collapseTableKey:[]});
    ...
    const loadHttp = (remoteUrl: any, pageNo = dtstate.page, pageSize = dtstate.rowsPerPage) => {
        setLoading(true);
        let fnUrl = '';
    ...
    const getTableHeadRow = ({ dk, index }: any) => {
        ...
        // if (dk.type === COLUMN_TYPE_STRING)
        return (
            <StyledTableCell key={index} align="center">{dk.name}
                {dk.sort === true &&
    ...
    const getCollapseTableHeadRow = ({ dk, index }: any) => {
        ...
        // if (dk.type === COLUMN_TYPE_STRING)
        return (
            <TableCell key={index} align="center">{dk.name}
            </TableCell>
    ...
    const mapCollapseTableList= (row:any) => {
        ...
        datastr.map((row:any) =>(
            ...
            <TableRow >
             ...
             getCollapseTableRow({ dk, row, keystr })
                                                             ...
                                                             actions.map((act, x) => (
                                                                  <Tooltip key={x} title={act.tip} placement="top" arrow>
                                                                     ...
                                                                     onClick={() => { 
                                                                       ...
                                                                       >
                                                                          {<Icon style={{ color: act.color }}>{act.icon}</Icon>}
                                                                      </IconButton>
                                                                  </Tooltip>
