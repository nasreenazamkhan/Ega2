...
import { Button, IconButton, Icon, TablePagination, TableSortLabel, TextField, Tooltip, CircularProgress } from '@material-ui/core';
import './table.css';
...
const DataTable: React.FC<TableProps> = ({ tableData, tableKeys, handleClick, actions = [], remote, remoteUrl, refresh, dataRootKey }) => {

    const [dtstate, setdtstate] = useState({ rowsPerPage: 10, page: 0, sortColumn: null, currentPageRows: [], currentPageKey: [], refresh: 0 });
                                                ...
                                                <StyledTableCell align="right" style={{ width: '120px' }}>
                                                    {
                                                        actions.map((act, x) => (
                                                            <Tooltip key={x} title={act.tip} placement="top" arrow>
                                                                <IconButton key={x} style={{ padding: "0px" }} onClick={() => { handleClick(row, extIndex, x, act) }}>
