...
const getTableRow = ({ dk, row, index }: any) => {
    ...
    if (dk.type === 0)
        return (
            <StyledTableCell key={index}>{row[dk.key]}</StyledTableCell>
        )
    else
        return (
            <StyledTableCell align="right" key={index}>{row[dk.key]}</StyledTableCell>
        )
...
const getTableHeadRow = ({ dk, index }: any) => {
    if (dk.type === 0)
        return (
            <StyledTableCell key={index}>{dk.name}</StyledTableCell>
        )
    else
        return (
            <StyledTableCell key={index} align="right">{dk.name}</StyledTableCell>
        )

}


const NormalTable: React.FC<TableProps> = ({ tableData, tableKeys, actions, handleClick, chkbox }) => {
                                            ...
                                            <StyledTableCell align="right" className="actions-col">
                                                {
                                                    actions.map((act: any, indx) => (
                                                        <Tooltip key={indx} title={act.tip} placement="top" arrow>
                                                            <IconButton key={indx} style={{ padding: "0px" }} onClick={() => { handleClick(row, extIndex, act) }}>
