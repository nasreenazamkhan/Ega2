...
import { IconButton, Icon, TablePagination, TableSortLabel, Tooltip, CircularProgress, Checkbox, Grid, Typography, Avatar, Radio } from '@material-ui/core';
...
const CollapseDataTable: React.FC<TableProps> = ({
    tableData, tableKeys, handleClick, actions = [], remote, remoteUrl, refresh, dataRootKey, chkbox = false, collapseChkBox = false, collapsableTableKeys = [],
    collapseTableList, keyTest, countData, onFilterSelected, selectedData, groupBy, showLinks = false, screen, updateChild }) => {
    ...
    const getCollapseTableRow = ({ dk, row, index }: any) => {
        console.log("inside getCollapseTableRow for key " + dk.key);
        let rd = 'row.' + dk.key;
        let datastr = eval(rd);
    ...
    const getTableRow = ({ dk, row, index }: any) => {
        ...
        if (dk.key === 'status' && datastr === 'Expired') {
            return (<StyledRedTableCell key={index} align="center">{datastr}</StyledRedTableCell>)
        }
        if (dk.key === 'status' && datastr === 'Active') {
            return (<StyledBlueTableCell key={index} align="center">{datastr}</StyledBlueTableCell>)
        }
        else if (dk.key === 'location') {
            return (<StyledFixedCell key={index} align="center">{datastr}</StyledFixedCell>)
        }
            ...
            return (<StyledTableCell key={index} align="left" style={{ color: row.openCollapsePanel ? '#fff' : '', textAlign:'left' }}>
                <div style={{
                    display: 'inline-flex',
                    boxSizing: 'inherit',
                    textAlign: 'center',
            ...
            return (
                <StyledTableCell align="left" key={index} style={{textAlign:'left', paddingLeft:'16px'}}>
                    <Link
                        style={{ textDecoration: "underline", color: row.openCollapsePanel ? '#fff' : '' , textAlign:'left'}}
                        onClick={() => {
        ...
        else if (dk.key.toLowerCase().includes("amount")) {
            return (<StyledTableCell align="left" key={index} style={{ color: row.openCollapsePanel ? '#fff' : '' }}>{datastr}  AED</StyledTableCell>)
        }
        else if (dk.key === 'referenceNumber') {
            return (<StyledTableCell align="left" key={index} style={{ color: row.openCollapsePanel ? '#fff' : '', textAlign: 'left' , paddingLeft:'16px'}}>{datastr}</StyledTableCell>)
        }
        else
            return (<StyledTableCell align="left" key={index} style={{ color: row.openCollapsePanel ? '#fff' : ''}}>{datastr}</StyledTableCell>)
    }
    ...
    const setCurrentSortColumn = () => {
        ...
        sortEle.map((ele) => ele.sortDir = 'asc');
    ...
    const handleSort = (scol: any) => {
        ...
        if (scol.sortDir === 'asc') scol.sortDir = 'desc';
        else scol.sortDir = 'asc';
        currentSortColum.sortDir = scol.sortDir;
        currentSortColum.sorted = true;
        let fnlUrl = null;
        if (remoteUrl.indexOf('?') === -1)
            fnlUrl = remoteUrl + '?sortCol=' + scol.id + '&sortOrder=' + scol.sortDir;
        else
            fnlUrl = remoteUrl + '&sortCol=' + scol.id + '&sortOrder=' + scol.sortDir;
    ...
    const getTableHeadRow = ({ dk, index }: any) => {
        ...
        //             <Grid item xs={8} style={{ height: '2px' }}  >
        //                 <BlueTooltip title={"Sort Asc"} arrow placement="top">
        //                     <IconButton style={{ padding: '5px', paddingBottom: 0 }} onClick={() => {
        //                         dtstate.sortColumn.forEach((element: any) => {
        //                             element.dir = ''
        //                         });
        //                         dk.dir = 'asc';
        //                         ref1.current.click()
        //                     }}>
        //                         {dk.dir == 'asc' ? <img src="./sort_asc.svg" height="22px" /> :
        //                             <img src="./sort_up.svg" height="22px" />}
        //                     </IconButton>
        //                 </BlueTooltip>
        //             </Grid>
        //             <Grid item xs={8} style={{ height: '2px', marginBottom: '10px' }}  >
        //                 <BlueTooltip title={"Sort Desc"} arrow placement="bottom">
        //                     <IconButton style={{ padding: '2px', paddingTop: 0, paddingRight: '5px', paddingLeft: '5px' }} onClick={() => {
        //                         dtstate.sortColumn.forEach((element: any) => {
        //                             element.dir = ''
        //                         });
        //                         dk.dir = 'desc';
        //                         ref1.current.click()
        //                     }}>
        //                         {dk.dir == 'desc' ? <img src="./sort_desc.svg" height="22px" /> :
        //                             <img src="./sort_down.svg" height="22px" />}
        //                     </IconButton>
        //                 </BlueTooltip>
        //             </Grid>
        //         </Grid>
        ...
        const DropdownIndicator = () => {
            return (
                <Grid container>
                    <BlueTooltip title={dk.sortDir} arrow placement="top">
                        <IconButton style={{ marginBottom: '-20px', paddingTop: 5, paddingBottom: 5 }}>
                            <Grid item xs={12} style={{ width: 0 }}>
                                {dk.sortDir == 'asc' ? <img src="./sort_asc.svg" height="22px" /> :
                                    <img src="./sort_up.svg" height="22px" />}
                            </Grid>
                            <Grid item xs={12}>
                                {dk.sortDir == 'desc' ? <img src="./sort_desc.svg" height="22px" /> :
                                    <img src="./sort_down.svg" height="22px" />}
                            </Grid>
                        </IconButton>
                    </BlueTooltip>
        ...
        const renderHeadStyle = () => {
            if (screen == 'newJob' && i == 0)
                return 'center'
            else if (dk.name == 'Consignee Details' || dk.name == 'Declaration Number'|| dk.name == 'Booking Number')
                return 'left'
            else
                return 'center'
        }
        ...
        const paddingStyle = () => {
            if (dk.name == 'Consignee Details')
                return '40px'
            else if (dk.name == 'Declaration Number' || dk.name == 'Booking Number')
                return '16px'
            else
                return ''
        }
        ...
        return (
            <>
                <StyledTableCell key={index} style={{ textAlign: renderHeadStyle(), paddingLeft: paddingStyle() }}>{dk.name}
                    {dk.sortActive === true && dtstate.sortColumn[i] &&
                        <TableSortLabel 
                            ...
                            // ref={ref1}
                            style={{ width: '35px' }}
                            direction={dtstate.sortColumn[i].sortDir}
                            // direction={dtstate.sortColumn[i].sortDir === 'asc' ? 'desc' : 'asc'}
                            onClick={() => {
                                console.log("----1", dtstate.sortColumn);
                                console.log("----2", dk);
                                if (dtstate.sortColumn.filter((y: any) => y.sorted).findIndex((y: any) => y.id === dk.id) == -1) {
                                    var currentSortColum = dtstate.sortColumn.find((y: any) => y.sorted === true);
                                    if (currentSortColum) { currentSortColum.sortDir = 'asc'; currentSortColum.sorted = false }
                                }
                                handleSort(dk);
                            }}
    ...
    const getCollapseTableHeadRow = ({ dk, index }: any) => {
        return (
            <CollapseTableCell key={index} align="center">{dk.name}</CollapseTableCell>
        )
    }
    ...
    const handleChangePage = (event: any, newpage: number) => {
        if (remote !== true) {
            ...
            if (currentSortColum !== undefined) {
                if (remoteUrl.indexOf("?") === -1)
                    fnlUrl = remoteUrl + "?sortCol=" + currentSortColum.id + "&sortOrder=" + currentSortColum.sortDir;
                else
                    fnlUrl = remoteUrl + "&sortCol=" + currentSortColum.id + "&sortOrder=" + currentSortColum.sortDir;
            } else
                fnlUrl = remoteUrl;
                ...
                <HeaderTableRow key={extIndex} selected={open}>
                    {chkbox === true ?
                        <StyledTableCell style={{
                            ...
                            {screen === 'newJob' && <Radio
                                icon={<img src="./radio-disabled.svg" height="23px" />}
                                checkedIcon={<img src="radio-enabled.svg" height="23px" />}
                                checked={row.referenceNumber !== radioSelected ? false : true}
                                onClick={(evnt: any) => {
                                    console.log("radio test ::", row);
                                    setRadioSelected(row.referenceNumber);
                                    setRerender(reRender + 1);
                                    handleClick(row, extIndex, 'collapseRowHeaderChecked', evnt);
