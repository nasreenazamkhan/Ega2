...
const HeaderTableRow = withStyles((theme: Theme) =>
    createStyles({
        ...
        selected:{
            ...
            backgroundColor: '#0E1B3D !important',
        }
    })
)(TableRow);
...
const CollapseDataTable : React.FC<TableProps> = ({ tableData, tableKeys, handleClick, actions = [], remote, remoteUrl, refresh, dataRootKey,chkbox=false,collapseChkBox=false,collapsableTableKeys=[],
    ...
    const getCollapseTableRow = ({ dk, row, index }: any) => {
        let rd = 'row.' + dk.key;
        let datastr = eval(rd);
        if((dk.key==='container_number'||dk.key==='dropZoneLabel') &&  showLinks)
        {
...
const getTableRow = ({ dk, row, index }: any) => {
    ...
    {
        return (<StyledTableCell key={index} align="left" >
          <Avatar style={{ height: '25px', width: '25px' ,backgroundColor:'#36A4F8',fontSize:'14px',marginLeft:"10%"}} > {datastr}</Avatar>
           </StyledTableCell>) 
    }
    else if(dk.key==='consigneeDetails' )
    {
       const initials= datastr.substring(datastr.indexOf('/') + 1);
        ...
        <Avatar style={{ height: '25px', width: '25px' ,backgroundColor:'#FF7F7B',fontSize:'14px'}} >{initials}
                </Avatar>
    ...
    if(dk.key==='transporterAmount' )
    {
        return (<StyledRedTableCell key={index} align="left" >{datastr + " AED"}</StyledRedTableCell>) 
    }
    else
