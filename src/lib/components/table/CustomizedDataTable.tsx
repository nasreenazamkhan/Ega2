...
import { ArrowDownwardRounded, StarRate } from "@material-ui/icons";
...
tableData,tableKeys,handleClick,actions = [],remote,remoteUrl,refresh,dataRootKey,countData,screen,onFilterSelected}) => {
  ...
  function GetTableRow({ dk, row, index, handleClick }: any) {
    ...
    if(dk.key === 'haulierName'){
      return(
        <StyledFixedCell key={index} align="left" className={classes.haulierName}>
            {datastr}
        </StyledFixedCell>
      )
    }
    ...
    if (dk.key === "statusCode") {
      ...
      if (datastr === "CNCL") {
        ...
        return (<StyledFixedCell key={index} align="center"></StyledFixedCell>)
      }
    }
  ...
  const loadHttp = (remoteUrl: any, pageNumber?: number, pageSize = dtstate.rowsPerPage) => {
    ...
    getHttp({ url: fnUrl }, false).then(e => {
      ...
      countData(e?.count ? e?.count : e?.totalElements);
      setLoading(false);
  ...
  const handleSort = (scol: any) => {
    ...
    else fnlUrl = remoteUrl + "&sortCol=" + scol.id + "&sortOrder=" + scol.sortDir;
    // if(scol.sortDir)
  ...
  //                     <IconButton style={{ padding: '2px', paddingTop:0 }} onClick={() => {
  //                       dtstate.sortColumn.forEach((element:any) => {
  //                         element.sortDir=''
  //                       });
  //                         dk.sortDir = 'desc';
  ...
  // }
    ...
    return (
      <>
        <StyledTableCell key={index} align="center" style={{textAlign: dk.key === 'haulierName' ? 'left' : 'center' , paddingLeft: dk.key === 'haulierName' ?'60px' : ''}}>
          {dk.name}
