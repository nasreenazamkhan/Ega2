...
class RequestBoeService {
   ...
   fetchInvoicesForPayment(referenceNumber)
   {
    const remoteUrl = `${endpointContants.fetchInvoicesForPayment}?referenceNumber=${referenceNumber}`;
   ...
   makePaymentForInvoices(request)
   {
    const remoteUrl = `${endpointContants.makePaymentForInvoices}`;

    let obj = { url: remoteUrl ,body:request};
    return postHttp(obj, true).catch(error => {
        return error;
    });
   }
