...
function Status() {
  ...
  const [payment, setPayment] = useState(location?.state?location.state.payment:null);
  ...
  return (
    <>
      ...
      <CustomizedDataTable
        ...
        handleClick={(row, index, action, element) => {
          BookingService.fetchPaymentSummary(
            ...
            if(tabSelected==='MISCELLANEOUS INVOICES')
            history.push('./importerPayment',{statusData: row ,paymentSummary:response, url:requestDetailsUrl});
            else     
      ...
      />
      {payment && <PaymentSuccessPopUp 
      referenceNumber={location.state.referenceNumber}
      success={location.state.success}></PaymentSuccessPopUp>
