...
function MiscellaneousInvoices() {
  ...
  useEffect(() => {
    RequestBoeService.fetchAllInvoices(bookingReferenceNumber).then((response) => {
     ...
     var rejectedItems=response.data.dataItems.filter(x=>x.status==='INVREJECT');
     var invoiceNumber=rejectedItems.size();
  ...
  return (
    ...
    <Table>
     ...
     { submittedInvoices.map((submittedInvoice)=>
     <>
     <TableRow key={submittedInvoice.referenceNumber}>
       {submittedInvoice.status!=='INVREJECT' && <Grid item container xs={12} sm spacing={1}>
            <Grid item xs={4} sm={2}>
                <LabelHeader
                >
                Invoice Number
