...
function TransporterStatusDetails(props) {
 ...
 if(props?.requestDetailsData)
 {
  ...
  useEffect(() => {
      ...
      containerInProgress = location.state.statusData.containerList.filter( (container) =>
      container.refStatus.code ==="INPRO"
  );
  containerDelivered =  location.state.statusData.containerList.filter( (container) =>
  container.refStatus.code ==="FCL_DEL"
);
containerCompleted = location.state.statusData.containerList.filter( (container) =>
container.refStatus.code ==="MT_DEL"
  ...
  setContainersCompleted(containerCompleted);
    ...
    // });
  }, [location.state.statusData.containerList]);
