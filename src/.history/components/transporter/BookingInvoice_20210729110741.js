...
function BookingInvoice(props) {
  ...
  useEffect(() => {
    RequestBoeService.fetchAllInvoices(bookingReferenceNumber).then((response) => {
     console.log(response);
      setInvoice(response?.data?.dataItems);
      invoice.invoiceDocSize=0;    
      setInvoice(rejectedItems);
  ...
  const deleteInvoiceDocs=(fileName)=>
  {
    ...
    Send For Approval
     ...
     {submittedInvoices && submittedInvoices.filter(x=>x.status!=='Invoice Rejected').map((submittedInvoice)=>
     <>
            ...
            <Grid item xs={4} sm={2}>
              ...
              <LabelHeader   
                ...
                Transporter Comments
                 ...
                 <LabelHeader   
                 >
                  {submittedInvoice.transporterRemarks}
                 </LabelHeader> 
