...
function PendingJobs(props) {
  ...
  const [confirmJobpopup,setConfirmJobPopup] = useState(true);
...
<Dialog
     ...
     h                      setConfirmJobPopup(NO_DIALOG)
   ...
   > Pending Jobs </Link> </p>
                        ...
                        <p style={{color:"grey"}}>Check the token status in  <Link color="secondary" 
                        onClick={()=> { setConfirmJobPopup(NO_DIALOG);   history.push("/newJobs",{tabSelected:2})
                           }}
