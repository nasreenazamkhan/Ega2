...
function BookingInvoice(props) {
  ...
  const onSubmit = (data) => {
    ...
    invoice.missingInvoiceError=false;

    if (invoice.fileName === undefined || invoice.fileName === '') {
      invoice.missingInvoiceError = true;
      validationSuccess = false;
    }
   if(validationSuccess)
    uploadInvoices(invoice);
                ...
                <div className="col-md-8">
                  {invoice &&
                    (invoice.status === "Invoice Approved" ||
                      invoice.status === "Invoice Submitted"||invoice.status==="PAID"||invoice.status==="PAID IN ADVANCE") && (
                      <>
                          ...
                          <Grid item xs={6} sm={4}>
                            ...
                            <ApplnDatePicker
                              ...
                              onChange={(e) => {
                                ...
                                //methods.getValues().invoiceDate=e;
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  promiseData.then((result) => {
                                   ...
                                   if(!allowedFileTypes.some(
                                      (item) => fileType.split("/")[1] === item))
                                    {
                                      console.log("file type error");
                                      setShowToaster("FILE_TYPE_ERROR");    
                                    }
