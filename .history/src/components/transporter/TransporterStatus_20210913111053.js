...
function TransporterStatus() {
  ...
  const onSearch = (values) => {
    let finalURL =
      "/ptms/app/api/secure/requestDetails/search?option=ALL&toDate=" +
      values.toDate +
      "&fromDate=" +
      values.fromDate +
      "&consigneeName=" +
  ...
  const handleSearchClick = async (e, value, type) => {
    ...
    if (e.serviceType === "BOOKING") {
      finalURL =
        "/ptms/app/api/secure/requestDetails/search?option=ALL&bookingNumber=" +
        value;
      ...
      let finalURL =
        "/ptms/app/api/secure/requestDetails/search?option=ALL&containerNumber=" +
        value;
      console.log("url", finalURL);
      setRequestDetailsUrl(finalURL);
      setPstate(pstate + 1);
