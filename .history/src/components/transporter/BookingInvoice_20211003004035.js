...
function BookingInvoice(props) {
    ...
    console.log(props.showBooking);
       ...
       invoiceAmount:  Yup.string().test(
             "maxDigitsAfterDecimal",
             "Please enter valid amount ",
             (invoiceAmount) => /^[0-9.]*$/.test(invoiceAmount)    
           )
      //  vatAmount: Yup.string().required("Vat Amount is required"),
      //  invoiceTotalAmount: Yup.string().required("Invoice Total Amount is required"),
      //  transporterRemarks: Yup.string().required("Transporter Remarks is required"),

        //test('testInput', 'The field should have digits only', digitsOnly)
   ...
   const formValues = {
    ...
    //   invoiceAmount: Yup.number().test(
    //     "maxDigitsAfterDecimal",
    //     "number field must have 2 digits after decimal or less",
    //     (invoiceAmount) => /^\d+(\.\d{1,2})?$/.test(invoiceAmount)    
    //   )
    // });
...
//}
  ...
  <>
        ...
        <Grid container direction="row" spacing={5}>
          <Grid item sm={12} xs={12}>
            <Typography variant="subtitle1" style={{ fontSize: '16px', textAlign: 'center' }}>
              <b>{'Invoice cannot be raised until booking is completed'}</b></Typography>
          </Grid>
        </Grid>
      </Paper>
   ...
   <>
    ...
    { invoice && (invoice.status==='Invoice Rejected'|| invoice.status===undefined ) && <Button
            variant="contained"
          ...
          <Grid item container xs={12} sm spacing={1}>
            <Grid item xs={6} sm={4}>
                ...
                <TextField
                            ...
                            error={methods.errors?.invoiceNumber ? true : false}
                            helperText={methods.errors?.invoiceNumber?.message ?methods.errors.invoiceNumber.message :""}
                          />
              ...
              <LabelHeader
                 ...
                 <TextField
                            ...
                            size="small"
                             ...
                             helperText={methods.errors?.invoiceAmount?.message ?methods.errors.invoiceAmount.message :""}
                             onChange={(e) => {
                               invoice.invoiceAmount = e.target.value;
                               formValues.invoiceAmount = e.target.value;
              ...
              <LabelHeader   
                ...
                Transporter Remarks (if Any)
                 ...
                 <TextField label="Enter Remarks" variant="outlined" style ={{width: '100%'}}
                  multiline="true"
   ...
   (
    ...
    <Link style={{fontSize:'16px',color:'#000000',fontFamily:'Dubai Regular',fontWeight:'bold'}}>{invoiceDoc.fileName}
      ...
      <FiberManualRecordIcon style={{fill:'#FF8E0D',marginRight:'3px'}}></FiberManualRecordIcon> 
          ...
          <Grid item>
      <InputLabel style={{color:'#FF8E0D',marginTop:'5px'}}> {invoice.status}</InputLabel>
      </Grid>
