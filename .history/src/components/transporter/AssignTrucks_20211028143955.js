...
export default function AssignTrucks() {
  ...
  // const [containersUnassigned, setContainersUnassigned] = useState({count:containerData.containerList.length});
  ...
  const [render, setShowConfirmation] = useState(NO_DIALOG);
  ...
  const updateContainerList = (e, containerNumber, transporter) => {
    ...
    if (container.assignedTruck) {
      if (e === "") setContainersUnassigned(prevState => ({
        ...prevState,
        count: containersUnassigned + 1
      }));
    } else {
      setContainersUnassigned(prevState => ({
        ...prevState,
        count: containersUnassigned - 1
      }));
  ...
  return (
    <>
      ...
      <CustomDialog
        ...
        onConfirm={(e) => {
          var time = e;
          var success=validateContainerData(containerList);
