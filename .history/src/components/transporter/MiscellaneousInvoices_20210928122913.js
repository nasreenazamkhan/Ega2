...
function MiscellaneousInvoices(props) {
  ...
  const [maxLengthMsg,setMaxLengthMsg]= useState([]);
  ...
  const validateInvoices = (invoiceList) => {
    ...
    for (let i = 0; i < invoiceList.length; i++) {
      invoiceList[i].vatAmountError = '';
      invoiceList[i].invoiceAmountError = '';
      invoiceList[i].totalInvoiceAmtError = '';
      invoiceList[i].missingInvoiceError = '';
      var regexp = /^[0-9.]*$/;
      if (!regexp.test(invoiceList[i].vatAmount)) {
        invoiceList[i].vatAmountError = true;
        validationSuccess = false;
      }
      if (!regexp.test(invoiceList[i].invoiceAmount)) {
        invoiceList[i].invoiceAmountError = true;
        validationSuccess = false;
      }
      if (!regexp.test(invoiceList[i].invoiceTotalAmount)) {
        invoiceList[i].totalInvoiceAmtError = true;
        validationSuccess = false;
      }
      if (invoiceList[i].fileName === undefined || invoiceList[i].fileName === '') {
        invoiceList[i].missingInvoiceError = true;
        validationSuccess = false;
      }
    }
                  ...
                  <Grid item container xs={12} sm spacing={1}>
                    ...
                    <Grid item xs={6} sm={2}>
                      ...
                      <TextField label="Enter Description" variant="outlined" style={{ width: '100%' }} multiline="true"
                        ...
                        helpertext={maxLengthMsg[invoice.serialNumber]}
                        // onBlur={(e) => {
                        ...
                        onChange={(e) => {
                          ...
                          setMaxLengthMsg[invoice.serialNumber]=250 - e.target.value.length + ' characters left.'
                          else
                            setMaxLengthMsg[invoice.serialNumber]=''
                        }}
                      ></TextField>
