...
function MiscellaneousPayment(props) {
  ...
  const [allContainerList, setAllContainerList] = useState(props?.paymentSummary?.containerList);
  const [tabSelected, setTabSelected] = useState(props.statusData.statusCode !== "MT_DEL" ? "In Progress" : "Completed");
  const [containerList, setContainerList] = useState(props.paymentSummary.containerList);
      ...
      if (["PPAY", "FPAY", "SUCC", "EXP", "MT_DEL"].includes(statusData.statusCode)){
        ...
        if (tabSelected === "In Progress") {
          let containerList = response.containerList.filter(
            (container) => 
            ["PEND", "PENTRUCK", "PODUPL", "INPRO", "PTOK", "PODREJ", "CNCL", "PODIMPAPPR", "CONF", "TRUCK_ASGN"].includes(container.refStatus.code)
          );
            ...
            onSelected={(e) => {
              if (e === 0) {
                let list = allContainerList?.filter(
                  (container) =>
                    ["PEND", "EXP", "PENTRUCK", "TRUCK_ASGN", "PTOK", "CONF", "INPRO", "PODUPL", "PODREJ", "PODIMPAPPR"].includes(container.refStatus.code)
              ...
              if (e === 1) {
                setContainerList(allContainerList?.filter(
                  (container) => ["FCL_DEL", "PMTTOK", "MTTOKASGN"].includes(container.refStatus.code)
            ...
            <Table >
              <TableHead style={{ backgroundColor: '#696F83', color: '#FFFFFF', paddingBottom: 0 }}>
                <TableRow>
                  ...
                  <StyledTableCell2>Drop Details</StyledTableCell2>
                  {["TRANSCONF", "STARTED", "MT_DEL"].includes(statusData.statusCode) && <>
                    <StyledTableCell2>
                      POD
                    </StyledTableCell2>
                    <StyledTableCell2 style={{ borderTopRightRadius: '10px' }}>
                      Track
                    </StyledTableCell2>
                  </>}
                  {(statusData.statusCode === "SUCC" || statusData.statusCode === "EXP") && (
              ...
              <TableBody>
                {
                  containerList.map((containers, inx) => (
                    <ContainerTableRow key={containers.container_number}>
                      ...
                      <StyledTableCell2>
                        <img src="./calendar.svg" style={{ marginRight: "8px" }} />
                      ...
                      {(["TRANSCONF", "STARTED", "MT_DEL"].includes(statusData.statusCode) && <StyledTableCell2>
                        <BlueTooltip title={containers.refStatus.code === "PODUPL" ? "Click here to verify POD" : "Click here to view approved POD"} placement="top-end" arrow>
                          <IconButton onClick={(e) => {
                            ...
                            <img src="./pod_pending.svg" />}
                            {["PODIMPAPPR", "FCL_DEL", "MT_DEL"].includes(containers.refStatus.code) &&
                              <img src="./pod_approved.svg" />}
                          </IconButton>
                        </BlueTooltip>
                      </StyledTableCell2>)}
                      {["TRANSCONF", "STARTED", "MT_DEL"].includes(statusData.statusCode) &&
 ...
 {payment && <PaymentConfirmation
      bookingNumber={location.state.bookingNumber}
