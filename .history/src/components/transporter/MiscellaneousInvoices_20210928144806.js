...
function MiscellaneousInvoices(props) {
  ...
  const validateInvoices = (invoiceList) => {
    ...
    for (let i = 0; i < invoiceList.length; i++) {
      ...
      if(inv.invoiceNumber==='' && inv.invoiceDate===''  &&  inv.description==='' && inv.vatAmount===undefined  && inv.invoiceAmount===undefined
        && inv.invoiceTotalAmount===undefined)
        {
          continue; 
        }
        else
      ...
      var regexp = /^[0-9.]*$/;
      if (!regexp.test(inv.vatAmount)) {
        inv.vatAmountError = true;
        validationSuccess = false;
      }
      if (!regexp.test(inv.invoiceAmount)) {
        inv.invoiceAmountError = true;
        validationSuccess = false;
      }
      if (!regexp.test(inv.invoiceTotalAmount)) {
