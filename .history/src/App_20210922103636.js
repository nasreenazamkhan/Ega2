...
const invoiceUpload =  lazyComponentLoad(() => {
  return import('./components/transporter/InvoiceUpload');
});

const tokenCount =  lazyComponentLoad(() => {
  return import('./components/admin/TokenCount');
});

const haulierList =  lazyComponentLoad(() => {
  return import('./components/admin/haulierListing/HaulierDetails');
});

const trackDetails =  lazyComponentLoad(() => {
  return import('./components/stepper/Track');
});

const importerPayment=  lazyComponentLoad(() => {
  return import('./components/transporter/ImporterPayment');
});

const paymentConfirmation=  lazyComponentLoad(() => {
  return import('./components/request/PaymentConfirmation');
});

const miscellaneousPayment=  lazyComponentLoad(() => {
  return import('./components/transporter/MiscellaneousPayment');
});
        ...
        {userLogin === true ?
          <Container>
            ...
            <Switch>
              ...
              <Route path="/invoiceUpload" exact component ={invoiceUpload}/>
              <Route path="/tokenCount" exact component ={tokenCount}/>
              <Route path="/importerPayment" exact component ={importerPayment}/>
              <Route path="/haulier-list" exact component ={hauimport MiscellaneousPayment from './../.history/src/components/transporter/MiscellaneousPayment_20210921153724';
lierList}/>
              <Route path="/paymentConfirmation" exact component ={paymentConfirmation}/>
              <Route path="/track-details" exact component ={trackDetails}/>
              <Route path="/miscellaneousPayment" exact component ={miscellaneousPayment}/>
            </Switch>
          </Container >
          : <AppSkeleton />}
      </div>
