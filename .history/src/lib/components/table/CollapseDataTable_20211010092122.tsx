...
const CollapseDataTable: React.FC<TableProps> = ({ 
    ...
    const getTableRow = ({ dk, row, index }: any) => {
        ...
        if (dk.key === 'status' && datastr === 'Active') {
            ...
            return (<StyledTableCell key={index} align="center" style={{color: row.openCollapsePanel ? '#fff':''}}>
                <div style={{
                    display: 'inline-flex',
                    boxSizing: 'inherit',
                    textAlign: 'center',
        ...
        {
            return (<StyledTableCell align="center" key={index} style={{color: row.openCollapsePanel ? '#fff':''}}>{datastr}  AED</StyledTableCell>)
        }
        else
            return (<StyledTableCell align="center" key={index} style={{color: row.openCollapsePanel ? '#fff':''}}>{datastr}</StyledTableCell>)
    }
    ...
    const getTableHeadRow = ({ dk, index }: any) => {
        ...
        const DropdownIndicator = () => {
            return (
                <Grid container style={{ paddingLeft: '10px', marginBottom: 0 }} >
                    <Grid item xs={8} style={{ height: '3px' }}  >
                    ...
                    <Grid item xs={8} style={{ height: '3px', marginBottom: '10px' }}  >
        ...
        return (
            <>
                <StyledTableCell key={index} style={{textAlign:screen == 'newJob' && i == 0 ? 'left' : 'center'}}>{dk.name}
