...
const CollapseDataTable: React.FC<TableProps> = ({ 
    ...
    collapseTableList, keyTest, countData, onFilterSelected, selectedData, groupBy, showLinks = false, screen }) => {
    ...
    const [headerChecked, setHeaderChecked] = useState(selectedData.length===0?false:true);
    ...
    const getTableRow = ({ dk, row, index }: any) => {
        ...
        if (dk.key === 'status' && datastr === 'Active') {
            ...
            return (<StyledTableCell key={index} align="left" style={{color: row.openCollapsePanel ? '#fff':''}}>
                <div style={{
                    display: 'inline-flex',
                    boxSizing: 'inherit',
                    textAlign: 'center',
            ...
            return (
                <StyledTableCell align="left" key={index}>
                    <Link
                        style={{ textDecoration: "underline" ,color: row.openCollapsePanel ? '#fff':'', textAlign:'center'}}
                        onClick={() => {
    ...
    const getTableHeadRow = ({ dk, index }: any) => {
        ...
        //             <Grid item xs={8} style={{ height: '8px' }}  >
        ...
        //                 </BlueTooltip>
        //             </Grid>
        //             <Grid item xs={8} style={{ height: '8px', marginBottom: '5px' }}  >
        ...
        const renderHeadStyle = () => {
            if(screen == 'newJob' && i == 0)
                return 'left'
            else if(dk.name == 'Consignee Details')
                return 'left'
            else
                return 'center'
        }
        ...
        return (
            <>
                <StyledTableCell key={index} style={{textAlign:renderHeadStyle(), paddingLeft:dk.name == 'Consignee Details' ? '40px': ''}}>{dk.name}
                    {dk.sort === true && dtstate.sortColumn[i] &&
                        <TableSortLabel key={index + 1}
                            active={true}
                            ref={ref1}
                            style={{ width: '35px' }}
                                        ...
                                        <StyledTableCell style={{ width: '27px', paddingRight: '0px'}}>
                                            {screen !== 'newJob' && <Checkbox
                                                ...
                                                onClick={(evnt: any) => {
                                                    ...
                                                    dtstate.currentPageRows.forEach(function (item: any) {                                                        item.checked = !headerChecked;
